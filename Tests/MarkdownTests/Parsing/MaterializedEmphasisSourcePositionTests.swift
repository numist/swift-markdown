...
/// source span. The surviving content (`*5*`) is nonetheless byte-identical to a single source
/// range - `expandPrefixTabs` expands only the consumed indentation and leaves the content bytes
/// intact - so it must resolve to those literal source bytes and its inlines must carry source
/// positions, exactly like the space-indented (`- *5*`) and plain-text (`*\tfoo`) forms already do.
/// Columns are byte-projected (a tab counts as one byte) in the shipped, flag-off parser.
