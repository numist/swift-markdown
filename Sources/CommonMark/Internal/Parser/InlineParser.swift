...
extension BlockParser {
    ...
    mutating func stampInline(_ node: DocumentStorage.Index, _ start: Int, _ end: Int, content: borrowing ContentSpan) {
        ...
        if inlineSawBackslashHardBreak && storage.options.contains(.cmarkBugCompatibility) {
            ...
            // why: Quirk E's continuation re-indent (see `BlockParser.addLineSegment`) remaps a re-indented continuation line's surviving content to the block's fixed content column, which can push the run's mapped source offsets rightward past its own physical line's byte extent. cmark works in (line, column) space, so it keeps the run on its own physical line (its re-indented column + width); the rewrite's byte-offset end (`lastByte + 1`) instead projects onto a LATER physical line - e.g. `> bar\nbaz\nqux` flag-ON: `baz`'s re-indented last content byte projects to line 3's start byte, so `lastByte + 1` reports `@3:2` for cmark's `@2:6`. Anchor the end on the run's BASE source line (`runBase` = the segment's re-indent base, via `sourceRunBase`), NOT on the start byte `s`: when smart-punct / entity substitution SPLITS a re-indented continuation into an arena glyph node plus a following source fragment (` - b\n  -- c\n  d`: `--`→`–` then ` c`), the fragment's own start `s` has ALREADY mapped past the line boundary, so a start-line-anchored guard misses the overshoot and the fragment's byte end (`@3:3`) leaks through `consolidateTextNodes` as the merged node's end. `runBase` still sits on the run's physical line, so the guard fires and the explicit end lands on that line at `startColumn + contentWidth` (`@2:10`), matching cmark and the un-split single-node case (an un-split `-- c` starts on line 2, so `s`'s line already worked). `startColumn` is the run's first byte measured from `runBase`'s line start, so it stays a correct flat column even when `s` overshot. The `isReindentedRun` guard restricts this correction to a genuinely re-indented run (`seg.sourceOffset != seg.offset`): a multi-line CONTIGUOUS segment (`a\nb`, `sourceOffset == offset`, from a top-level paragraph's source-adjacent lines) spans several physical lines under ONE `runBase` on its FIRST line, so an interior-line run's byte projection is already exact while the overshoot comparison would mis-fire against that first-line base and stamp an inverted (position-less) range - reporting it not-re-indented keeps the exact byte projection. The `!runContainsNewline` guard restricts this to a single-physical-line TEXT run (a re-indented continuation text run carries no interior newline - the joining soft break is a separate node), so `start` and `end - 1` share one segment and one `runBase`; a genuinely multi-line WRAPPER node whose closer overshoots is handled by the next branch (which also anchors on `runBase`). When `s` and `runBase` share a line (the common re-indented run) this is byte-identical to anchoring on `s`. Flag-OFF keeps the byte-projected end (spec-correct).
            let startLine = sourceLineNumber(ofSource: runBase)
            let startColumn = s - lineStartByte(ofSource: runBase) + 1
            // why: a MULTI-LINE inline (code span / emphasis) preceding this text run on the segment's FIRST physical line consumes that line within the same re-indented segment, so the run's START byte `s` maps past the line boundary too - it overshoots onto a LATER physical line exactly as `lastByte + 1` does (`> `\n`o\nx` flag-ON: after the multi-line `` ` ``…`` ` `` code span, `o`'s re-indented `s` projects to line 3's start byte, so the byte-projected start reports `@3:1` while cmark reports `@2:4`). Anchor the START on `runBase`'s line at its own re-indented column - the same flat `startColumn` the end uses - so the run stays whole on its physical line; leaving the start byte-projected while the end is corrected onto `runBase`'s line inverts the range (start `@3:1` > end `@2:5`), which renders position-less. When `s` does NOT overshoot (the common re-indented run, `s` on `runBase`'s line), this explicit start equals the byte projection (byte-identical to before). Flag-OFF keeps the byte-projected start (spec-correct).
